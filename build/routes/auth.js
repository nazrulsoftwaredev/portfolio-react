...
import { ensureBootstrapAdmin, markLoginSuccess, validateAdminCredentials, } from "../services/authService.js";
import { clearAuthCookies, setAuthCookies, signAccessToken, signRefreshToken, verifyRefreshToken, } from "../utils/auth.js";
...
const toSecurityPayload = (settings) => ({
...
const resolveLoginCredentials = (req) => {
    const body = req.body && typeof req.body === "object"
        ? req.body
        : {};
    const basicAuth = parseBasicAuth(Array.isArray(req.headers.authorization)
        ? req.headers.authorization[0]
        : req.headers.authorization);
    return {
        email: typeof body.email === "string" ? body.email : (basicAuth?.email ?? ""),
...
router.post("/refresh", async (req, res) => {
    ...
    try {
        ...
        return sendError(res, 401, "UNAUTHORIZED", "Invalid or expired refresh session");
    }
});
...
const updateProfileHandler = async (req, res, next) => {
    try {
        ...
        if (!parsed.success) {
            return sendError(res, 400, "VALIDATION_ERROR", "Invalid profile payload", parsed.error.flatten());
        }
        const user = await UserModel.findById(authUserId);
        if (!user || !user.isActive) {
            return sendError(res, 401, "UNAUTHORIZED", "Session is not valid");
        ...
        if (parsed.data.email) {
            ...
            if (emailInUse) {
                return sendError(res, 409, "CONFLICT", "Email address is already in use");
            }
...
router.get("/security", requireAuth, async (req, res, next) => {
    try {
        ...
        return sendSuccess(res, 200, toSecurityPayload({
            twoFactorEnabled: typeof user.twoFactorEnabled === "boolean"
                ? user.twoFactorEnabled
                : defaultSecuritySettings.twoFactorEnabled,
            loginNotifications: typeof user.loginNotifications === "boolean"
...
const updateSecurityHandler = async (req, res, next) => {
    try {
        const authUserId = req.authUser?.id;
        if (!authUserId) {
            return sendError(res, 401, "UNAUTHORIZED", "Authentication required");
        ...
        if (!parsed.success) {
            return sendError(res, 400, "VALIDATION_ERROR", "Invalid security payload", parsed.error.flatten());
        }
        const user = await UserModel.findById(authUserId);
        if (!user || !user.isActive) {
            return sendError(res, 401, "UNAUTHORIZED", "Session is not valid");
        ...
        const nextSettings = {
            ...
            loginNotifications: parsed.data.loginNotifications ??
        ...
        await user.save();
        return sendSuccess(res, 200, toSecurityPayload(nextSettings), "Security settings updated");
    }
    catch (error) {
        return next(error);
    }
