...
import { createClient, deleteClient, getClientById, listClients, updateClient, } from "../services/clientService.js";
...
const toPaged = (items, page, pageSize) => {
...
const toPagedMeta = (page, pageSize, totalItems) => {
    const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));
    const safePage = Math.min(page, totalPages);
    return {
        page: safePage,
...
const createClientHandler = async (req, res, next) => {
    try {
        ...
        if (!parsed.success) {
            return sendError(res, 400, "VALIDATION_ERROR", "Invalid client payload", parsed.error.flatten());
        }
...
const updateClientHandler = async (req, res, next) => {
    try {
        ...
        if (!idParsed.success) {
            return sendError(res, 400, "VALIDATION_ERROR", "Invalid client id", idParsed.error.flatten());
        }
        const payloadParsed = clientPatchSchema.safeParse(req.body);
        if (!payloadParsed.success) {
            return sendError(res, 400, "VALIDATION_ERROR", "Invalid client payload", payloadParsed.error.flatten());
        }
        const payload = {
            ...payloadParsed.data,
        ...
        const client = await updateClient(authUserId, idParsed.data.id, payload);
        if (!client) {
...
router.get("/", async (req, res, next) => {
    try {
        ...
        return sendSuccess(res, 200, { items: result.items.map(toClientPayload) }, undefined, {
            page: result.page,
...
router.get("/:id/projects", async (req, res, next) => {
    try {
        ...
        if (!pageParsed.success) {
            return sendError(res, 400, "VALIDATION_ERROR", "Invalid pagination query", pageParsed.error.flatten());
        }
        const client = await getClientById(authUserId, idParsed.data.id);
        if (!client) {
            return sendError(res, 404, "NOT_FOUND", "Client not found");
        ...
        return sendSuccess(res, 200, { items: projects.map(toProjectPayload) }, undefined, toPagedMeta(page, pageSize, totalItems));
    }
    catch (error) {
        return next(error);
    }
...
router.get("/:id/invoices", async (req, res, next) => {
    try {
        ...
        return sendSuccess(res, 200, { items: invoices.map(toInvoicePayload) }, undefined, toPagedMeta(page, pageSize, totalItems));
    }
    catch (error) {
        return next(error);
    }
...
router.get("/:id/messages", async (req, res, next) => {
    try {
        ...
        return sendSuccess(res, 200, { items: messages.map(toMessagePayload) }, undefined, toPagedMeta(page, pageSize, totalItems));
    }
    catch (error) {
        return next(error);
    }
...
router.get("/:id/activity", async (req, res, next) => {
    try {
        ...
        const paged = toPaged(activity, pageParsed.data.page, pageParsed.data.pageSize);
...
router.post("/:id/messages/:messageId/reply", async (req, res, next) => {
    try {
        ...
        if (!idParsed.success || !messageId.success) {
            return sendError(res, 400, "VALIDATION_ERROR", "Invalid message reply target", {
        ...
        if (!payloadParsed.success) {
            return sendError(res, 400, "VALIDATION_ERROR", "Invalid message reply payload", payloadParsed.error.flatten());
        }
        const client = await getClientById(authUserId, idParsed.data.id);
        if (!client) {
            return sendError(res, 404, "NOT_FOUND", "Client not found");
...
router.delete("/:id", async (req, res, next) => {
    try {
        ...
        return sendSuccess(res, 200, { id: client._id.toString() }, "Client permanently deleted");
    }
    catch (error) {
        return next(error);
    }
