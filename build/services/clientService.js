...
const generateAvatar = (name, email) => {
    const seed = encodeURIComponent((name || email || "client").trim() || "client");
...
const calculateMomentum = (previousValue, nextValue) => {
...
export const ensureClientsSeed = async (ownerUserId) => {
...
export const listClients = async ({ ownerUserId, page, pageSize, q, status, sort, }) => {
    const ownerObjectId = new mongoose.Types.ObjectId(ownerUserId);
...
export const getClientById = async (ownerUserId, clientId) => {
    return ClientModel.findOne({
        _id: new mongoose.Types.ObjectId(clientId),
        ownerUserId: new mongoose.Types.ObjectId(ownerUserId),
    });
};
export const createClient = async (ownerUserId, input) => {
...
export const updateClient = async (ownerUserId, clientId, input) => {
    const filter = {
        _id: new mongoose.Types.ObjectId(clientId),
        ownerUserId: new mongoose.Types.ObjectId(ownerUserId),
    };
...
export const deleteClient = async (ownerUserId, clientId) => {
